...
class MoveNearInSceneEnv(CustomSceneEnv):
    ...
    def _setup_prepackaged_env_init_config(self):
        ...
        ret["rgb_overlay_path"] = str(
            ...
            # ASSET_DIR / "real_inpainting/google_move_near_real_eval_1_modify4.png"
        )
    ...
    def _initialize_actors(self):
        ...
        for i, obj in enumerate(self.episode_objs):
            ...
            # if obj.name != self.episode_source_obj.name and obj.name != self.episode_target_obj.name:
            #     thresh = 0.08
            #     # p = [p[0] + self._episode_rng.uniform(-thresh, thresh), p[1] + self._episode_rng.uniform(-thresh, thresh), p[2]]
            #     p = [p[0] - thresh, p[1] - thresh, p[2]]
