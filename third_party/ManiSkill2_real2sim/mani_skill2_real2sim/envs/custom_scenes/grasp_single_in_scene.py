...
class GraspSingleInSceneEnv(CustomSceneEnv):
    ...
    def _setup_prepackaged_env_init_config(self):
        ...
        ret["rgb_overlay_path"] = str(
            ...
            # ASSET_DIR / "real_inpainting/google_coke_can_real_eval_1_modify.png"
            # ASSET_DIR / "real_inpainting/google_coke_can_real_eval_1_modify4.png"
            # ASSET_DIR / "real_inpainting/google_coke_can_real_eval_1_modify14.png"
        )
    ...
    def _setup_lighting(self):
        ...
        if self.slightly_brighter_lighting:
            ...
            self._scene.add_directional_light([-1, -0.5, -1], color)
            self._scene.add_directional_light([1, 1, -1], color)
            ...
            self._scene.add_directional_light([0, 0, -1], color)
    ...
    def _initialize_actors(self):
        ...
        if not hasattr(self, "obj_init_quat_dict"):
            obj_init_rot_quat = self.obj_init_options.get("init_rot_quat", [1, 0, 0, 0])
        else:
            obj_init_rot_quat = self.obj_init_quat_dict.get(
                self.model_id, [1, 0, 0, 0]
            )
    ...
    def evaluate(self, **kwargs):
        ...
        # get the distance between the object and the gripper
        tcp_to_obj_dist = np.linalg.norm(self.obj_pose.p - self.tcp.pose.p)
        near_obj = tcp_to_obj_dist < 0.05
...
class GraspSingleRandomObjectInSceneEnv(GraspSingleCustomInSceneEnv):
...
class GraspSingleCokeCanDarkerInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def get_obs(self):
        ...
        for camera_name in self.rgb_overlay_cameras:
            obs['image'][camera_name]['Color'] = np.power(obs['image'][camera_name]['Color'], 2.0)
        return obs


@register_env("GraspSingleCokeCanDistractorInScene-v0", max_episode_steps=80)
class GraspSingleCokeCanDistractorInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def __init__(self, **kwargs):
        kwargs.pop("model_ids", None)
        kwargs["model_ids"] = ["opened_coke_can"]
        ...
        self.distractor_model_ids = distractor_model_ids
        kwargs["distractor_model_ids"] = distractor_model_ids
        super().__init__(**kwargs)
...
@register_env("GraspSingleCokeCanHardDistractorInScene-v0", max_episode_steps=80)
class GraspSingleCokeCanHardDistractorInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def __init__(self, **kwargs):
        kwargs.pop("model_ids", None)
        kwargs["model_ids"] = ["opened_coke_can"]
        distractor_model_ids = [
...
@register_env("GraspSingleRandomObjectDistractorInScene-v0", max_episode_steps=80)
class GraspSingleRandomObjectDistractorInSceneEnv(GraspSingleCustomInSceneEnv):
    def __init__(self, **kwargs):
        kwargs.pop("model_ids", None)
        self.all_model_ids = [
            "opened_coke_can",
        ...
        # }
        # random select
        # model_id = random_choice(all_model_ids, self._episode_rng)
        # kwargs["model_ids"] = [model_id]
        # # keep other objects as distractors
        # distractor_model_ids = all_model_ids.copy()
        # distractor_model_ids.remove(model_id)
        # self.distractor_model_ids = distractor_model_ids
        # kwargs["distractor_model_ids"] = distractor_model_ids
        super().__init__(**kwargs)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.set_episode_rng(seed)
        model_id = random_choice(self.all_model_ids, self._episode_rng)
        # keep other objects as distractors
        distractor_model_ids = self.all_model_ids.copy()
        distractor_model_ids.remove(model_id)

        options["model_id"] = model_id
        self.distractor_model_ids = distractor_model_ids
        options["distractor_model_ids"] = distractor_model_ids
        return super().reset(seed=seed, options=options)
...
@register_env("GraspSingleOpenedCokeCanDrawerVariantInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanDrawerVariantInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def _additional_prepackaged_config_reset(self, options):
        ret = super()._additional_prepackaged_config_reset(options)
        rgb_overlay_path = 'ManiSkill2_real2sim/data/real_inpainting/google_coke_can_real_eval_1_drawer_pos.png'
        self.rgb_overlay_img = cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
        return ret


@register_env("GraspSingleOpenedCokeCanLightVariantInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanLightVariantInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def _additional_prepackaged_config_reset(self, options):
        ret = super()._additional_prepackaged_config_reset(options)
        rgb_overlay_path = 'ManiSkill2_real2sim/data/real_inpainting/google_coke_can_real_eval_1_light_pos.png'
        self.rgb_overlay_img = cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
        return ret


@register_env("GraspSingleOpenedCokeCanTablePaperVariantInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanTablePaperVariantInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def _additional_prepackaged_config_reset(self, options):
        ret = super()._additional_prepackaged_config_reset(options)
        rgb_overlay_path = 'ManiSkill2_real2sim/data/real_inpainting/google_coke_can_real_eval_1_paper.png'
        self.rgb_overlay_img = cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
        return ret


@register_env("GraspSingleOpenedCokeCanTableStoneVariantInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanTableStoneVariantInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def _additional_prepackaged_config_reset(self, options):
        ret = super()._additional_prepackaged_config_reset(options)
        rgb_overlay_path = 'ManiSkill2_real2sim/data/real_inpainting/google_coke_can_real_eval_1_stone.png'
        self.rgb_overlay_img = cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
        return ret


@register_env("GraspSingleOpenedCokeCanTableStone2VariantInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanTableStone2VariantInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def _additional_prepackaged_config_reset(self, options):
        ret = super()._additional_prepackaged_config_reset(options)
        rgb_overlay_path = 'ManiSkill2_real2sim/data/real_inpainting/google_coke_can_real_eval_1_stone2.png'
        self.rgb_overlay_img = cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
        return ret


@register_env("GraspSingleOpenedCokeCanTableStone3VariantInScene-v0", max_episode_steps=80)
class GraspSingleOpenedCokeCanTableStone3VariantInSceneEnv(GraspSingleOpenedCokeCanInSceneEnv):
    def _additional_prepackaged_config_reset(self, options):
        ret = super()._additional_prepackaged_config_reset(options)
        rgb_overlay_path = 'ManiSkill2_real2sim/data/real_inpainting/google_coke_can_real_eval_1_stone3.png'
        self.rgb_overlay_img = cv2.cvtColor(cv2.imread(rgb_overlay_path), cv2.COLOR_BGR2RGB) / 255
        return ret
